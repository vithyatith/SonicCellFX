...
public class Butterworth extends IIRFilter {
     ...
     */
    public Butterworth(int order, double freq, boolean lowp) {
        //	super(source);
     ...
     */
    public Butterworth(int order, double freq1, double freq2, boolean pass) {
        //	super(source);
    ...
    public void configure(int order, double freq1, double freq2, boolean pass) {
        ...
        bb = computeB(n, ff1, ff2, pass);
        for (int i = 0; i < bb.length; ++i) {
            bb[i] *= scale;
        }

        aa = computeA(n, ff1, ff2, pass);
     ...
     * @return order (as int) and natural frequency (multiply by samplef/2 to
     * obtain the respective frequency)
     */
     ...
     * @param loss max loss in pass band, typically 3 (dB)
     * @param attn attenuation in the stop band, typically 60 (dB)
     * @return order (as int) and natural frequencies (multiply by samplef/2 to
     * obtain the respective frequency)
     */
    public static double[] ord(double[] fp, double[] fs, double loss, double attn) {
        // warp frequencies
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (f2 == 0.) {
            bwf = new Butterworth(nn, f1, true);
        } else {
            bwf = new Butterworth(nn, f1, f2, true);
        }
