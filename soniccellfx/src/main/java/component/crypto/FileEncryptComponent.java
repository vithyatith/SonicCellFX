 ...
 * @author yada
 */
public class FileEncryptComponent extends ComponentAbstraction {

    private String inputFilename = "bin.bin";
    private String outputFilename = "out.bin";
    private boolean processingBool = false;
    private String password = "sonic123";

    public FileEncryptComponent() {
        setName("FileEncrypt");
        this.setProperty("inputFilename", inputFilename);
        this.setProperty("outputFilename", outputFilename);
        this.setProperty("password", password);
        this.addInput(new DataTypeIO("string", "File to encrypt", "inputFilename"));
    ...
    public Object onExecute() {
        ...
        if (processingBool) {
            ...
            try {
                boolean b = FileCrypto.encrypt(inputFilename, outputFilename, password);
                sendData(0, b);
            } catch (Exception e) {
                sendData(0, false);
                e.printStackTrace();
    ...
    public boolean start() {
        inputFilename = getProperty("inputFilename").toString();
    ...
    public void loadProperty(String key, Object val) {
        if (key.equalsIgnoreCase("inputFilename")) {
            inputFilename = (String) val;
        ...
        Platform.runLater(new Runnable() {
            ...
            public void run() {
                ...
                if (selectedFile != null) {
                    inputFilename = selectedFile.getAbsoluteFile().toString();
                    setProperty("inputFilename", inputFilename);
                    setProperty("outputFilename" , inputFilename+ ".encr");
